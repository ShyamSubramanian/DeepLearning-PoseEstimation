...
def _attention_iter(inputs, lrnSize, itersize, mult = True, name = 'attention_iter'):
    with tf.name_scope(name):
        ...
        if mult:
            ...
            pfeat = tf.concat(stacks, axis = 3)
    return pfeat
	
def _attention_part_crf(inputs, lrnSize, itersize, usepart, mult = True, name = 'attention_part'):
	with tf.name_scope(name):
		if usepart == 0:
			return _attention_iter(inputs, lrnSize, itersize, mult)
		else:
			partnum = 17
			pre = []
			for i in range(partnum):
				att = _attention_iter(inputs, lrnSize, itersize, mult)
