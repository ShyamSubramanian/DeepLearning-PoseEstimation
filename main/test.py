...
def test_net(tester, dets, det_range, gpu_id, is_vis = False):
    ...
    while img_start < det_range[1]:
        ...
        for batch_id in range(0, len(cropped_data), cfg.test_batch_size):
            ...
            if is_vis == False:
                ...
                imgs = cfg.denormalize_input(imgs)
                imgs = imgs.astype('uint8')
                np.save("img",imgs)
                tester.vis_attn([imgs])
                return
...
def visualizer(test_model,img):
    ...
    if cfg.useGTbbox and cfg.testset in ['train', 'val']:
        ...
        imgname = []
        for i in dets:
            imgname.append(i['image_id'])
        for i in range(len(dets)):
            dets[i]['imgpath'] = 'val2017/' + imgname[i]
    else:
        with open(cfg.human_det_path, 'r') as f:
            dets = json.load(f)
...
if __name__ == '__main__':
    def parse_args():
        ...
        parser.add_argument('--newtestdata', type=str, dest='newtestdata')
    ...
    args = parse_args()
    if not args.visatt:
        if args.newtestdata == "y":
            cfg.newtestdata = True
        test(int(args.test_epoch))
    else:
        visualizer(int(args.test_epoch),args.imgid)
