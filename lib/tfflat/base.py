...
class Tester(Base):
    ...
    def vis_attn(self,data):
        print("Visual Attention module...",len(self.net.attentions))
        feed_dict, batch_size = self.next_feed(data)
        i = 0
        j = 0
        for attn in self.net.attentions:
            i += 1
            if i < len(self.net.attentions)-1:
                units = self.sess.run(attn,feed_dict=feed_dict)
                print(type(units))
                self.plotNNFilter(units,i)
            else:
                units = self.sess.run(attn,feed_dict=feed_dict)
                print(type(units))
                self.plotNNFilter2(units,j)
                j+=256
    
    def plotNNFilter(self,units,n):
        print("Plot NN Filter...")
        #units_sum = np.sum(units,3, keepdims=True)
        plt.imshow(units[0,:,:,0])
        np.save("visatt/attn"+str(n),units[0,:,:,3])
        print("showing")
        
    def plotNNFilter2(self,units,n):
        print("Plot NN Filter...")
        #units_sum = np.sum(units,3, keepdims=True)
        print(n)
        print(units.shape)
        for i in range(units.shape[3]):
            np.save("visatt/partattn"+str(n+i),units[0,:,:,i])
        print("showing final")
